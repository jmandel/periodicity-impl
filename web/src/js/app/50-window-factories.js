  ...
  function bindCycleIGShareControls() {
    var cards = document.querySelectorAll("[data-cycle-ig-share-card]");
    for (var index = 0; index < cards.length; index++) {
      var card = cards[index];
      if (card.dataset.cycleIgControlsBound === "1") {
        continue;
      }
      card.dataset.cycleIgControlsBound = "1";
      card.addEventListener("click", function (event) {
        var copyButton = closestFromEvent(event, "[data-cycle-ig-copy]");
        var shareButton = closestFromEvent(event, "[data-cycle-ig-native-share]");
        var currentCard = this;
        var linkInput = currentCard.querySelector("[data-cycle-ig-link]");
        var status = currentCard.querySelector("[data-cycle-ig-action-status]");
        var link = linkInput ? String(linkInput.value || "").trim() : "";
        ...
        if (copyButton && currentCard.contains(copyButton)) {
          event.preventDefault();
          if (!link) {
            return;
          }
          writeTextToClipboard(link).then(function () {
        ...
        if (shareButton && currentCard.contains(shareButton)) {
          event.preventDefault();
          if (!link) {
            return;
          }
          if (navigator.share && typeof navigator.share === "function") {
            navigator.share({ title: "Ovumcy SMART Link", text: link }).then(function () {
          ...
          writeTextToClipboard(link).then(function () {
            setStatus("Sharing is not available here, so the link was copied.");
          }).catch(function () {
            setStatus("Sharing is not available and the link could not be copied.");
          });
        }
      });
    }
