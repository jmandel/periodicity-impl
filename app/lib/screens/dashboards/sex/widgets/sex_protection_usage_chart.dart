...
class SexProtectionUsageChart extends StatelessWidget {
  final List<SexLogEntry> historyEntries;

  const SexProtectionUsageChart({super.key, required this.historyEntries});
  ...
  Widget build(BuildContext context) {
    ...
    // Count occurrences of each protection type
    final counts = <SexProtectionTypes, int>{};
    for (var entry in historyEntries) {
      if (entry.protectionType != null) {
        counts[entry.protectionType!] = (counts[entry.protectionType!] ?? 0) + 1;
      }
    }
    ...
    return Column(
      ...
      children: [
        Text(l10n.sexActivityScreen_protectionDistribution,
          style: TextStyle(
            fontSize: 18,
