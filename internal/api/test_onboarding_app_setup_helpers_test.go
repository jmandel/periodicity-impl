...
type onboardingTestAppOptions struct {
	...
	cycleIGShares    *services.CycleIGShareClient
...
func newTestHandlerDependencies(database *gorm.DB, i18nManager *i18n.Manager, options ...onboardingTestAppOptions) Dependencies {
	...
	dependencies := bootstrap.BuildDependencies(db.NewRepositories(database), []byte("test-secret-key"), i18nManager, bootstrap.Options{
	...
	if appOptions.cycleIGShares != nil {
		dependencies.CycleIGService = services.NewCycleIGService(dependencies.DayService, dependencies.SymptomService, dependencies.SettingsService, appOptions.cycleIGShares)
	}
	return dependencies
}
