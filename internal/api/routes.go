...
func registerV1APIRoutes(app *fiber.App, handler *Handler) {
	...
	cycleIG := v1.Group("/cycle-ig", handler.AuthRequired, handler.OwnerOnly)
	cycleIG.Post("/sample", handler.LoadCycleIGSampleData)
	cycleIG.Post("/preview", handler.PreviewCycleIGShare)
	cycleIG.Post("/shares", handler.CreateCycleIGShare)
	cycleIG.Post("/shares/:id/revoke", handler.RevokeCycleIGShare)
}
